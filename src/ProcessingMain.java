...
public class ProcessingMain extends PApplet {
	...
	public void setup() {
		...
		source = new CSourceFire(num[0], num[1]);
	}
	...
	public void serialEvent(Serial myPort) {
		...
		if (myString != null) {
			...
			for (int i = 0; i < list.length; i++) {
				num[i] = parseWithDefault(list[i], 0);
	...
	class CParticleFire {
		...
		void update() {
			...
			float dx = (float) (2.0f * (0.5 - noise(perlinPosX)));
			float dy = (float) (2.0f * (0.5 - noise(perlinPosY)));
	...
	class CSourceFire {
		...
		ArrayList<CParticleFire> particles;
		...
		CSourceFire(float x, float y) {
			...
			particles = new ArrayList<CParticleFire>();
		}
		...
		void update(float x, float y, int n) {
			...
			for (int i = 0; i < n && particles.size() < MAX_PARTICLES; i++) {
				coef = random(1);
				particles.add(new CParticleFire(this.x * coef + this.lastx
						* (1.0f - coef), this.y * coef + this.lasty
						* (1.0f - coef)));
			}
		...
		void getImage(PImage img) {
			...
			for (int i = width * height - 1; i >= 0; i--) {
				img.pixels[i] = bufferA[i] << 24 | bufferR[i] << 16
						| bufferG[i] << 8 | bufferB[i];
			}
			img.updatePixels();
		}
	...
	void fastBlur(int[] buf) {
		...
		for (; id < lim; id += 1 + k + k) {
			for (int i = 1 + k; i < width - 1; i += 2, id += 2) {
				buf[id] >>= 1;
				buf[id] += (buf[id - 1] + buf[id + 1] + buf[id + width] + buf[id
						- width]) >> 3;
			}
